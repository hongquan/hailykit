...
| `--tdd` | Pass-through to `{skill:hc-cook}` — Red-Green for new-behavior phases (test committed failing, then implemented to green) or Snapshot for refactor/legacy phases; see `{skill:hc-cook}` `references/process-steps.md` § --tdd Flag Behavior |
...
4. **Execute** — set the loop-guard marker before entering the phase loop: append `export HL_LOOP_GUARD_ACTIVE=1` to `$CLAUDE_ENV_FILE`. While set, test/spec files and `diff-tests.sh` are tripwire-blocked + audit-logged for Edit/Write/MultiEdit/NotebookEdit (`kit/hooks/haily-lib/directory.cjs` `checkLoopGuardTripwire`) — a SECONDARY, agent-writable guard; the regression gate's test-name-set shrinkage check (`references/regression-gate.md`) is PRIMARY and catches a removed test regardless. Loop over phases in Stage Graph order; run parallel when `dependencies` allow:
   ...
   - **Apex direction consult (`--auto`, tier-gated):** under `--auto`, when an ambiguous direction decision would be a user checkpoint in interactive mode and `HL_MODEL_TIER` ranks below `ultra`, consult the `haily-advisor` agent instead of guessing — question package in (context, candidate directions, the one question, cited files), one recommendation out; at most once per run. Treat an unset or unrecognized `HL_MODEL_TIER` the same way — skip the consult and decide at session tier (the fail-safe convention `kit/hooks/haily-lib/subagent.cjs` applies to the same signal: an ambiguous tier value must never trigger an escalation). Record the recommendation in the ledger row for post-run audit. Session-model fallback with the notice `⚠ advisor unavailable — advice by session model`. The advisor decides technical direction only — critical blockers (data loss, security hole, broken public contract) still escalate to the user per § --auto Mode.
